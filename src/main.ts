...
import {
  prepareRawCommits,
  type PrepareRawCommitsParams,
} from "./utils/prepareRawCommits";
...
export async function run({
  ...
  outputs.commits = await prepareRawCommits({
    from: sources,
    payload,
    listCommitsAPI,
  });
  ...
  outputs.currentLabels = await (replace ? setLabelsAPI : addLabelsAPI)(
    outputs.labels,
  );
  return { outputs };
}

export interface RunParams extends Partial<
  Pick<PrepareRawCommitsParams, "payload" | "listCommitsAPI">
> {
  inputs?: Inputs;
